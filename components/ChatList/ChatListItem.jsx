...
export default function ChatListItem(props) {
    ...
    const menuButton = isHoverItem ?
        <div className={`flex-none rounded-full h-10 w-10 my-auto border border-gray-200 transition duration-300 ease-in-out bg-white hover:bg-gray-100`}
    ...
    return (
        <div className={`mx-1 p-2 w-96 h-16 rounded-lg transition duration-300 ease-in-out flex cursor-pointer bg-white ${!isHoverMenu ? 'hover:bg-gray-100' : ''}`}
            ...
            <div className='flex-1 font-sans text-neutral-800'>
                <div className='text-base font-medium'>{props.user}</div>
                <div className='text-sm font-normal text-neutral-600'>{props.lastMessage}</div>
            </div>
            {menuButton}
        </div>
    )
